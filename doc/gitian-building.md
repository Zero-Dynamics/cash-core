...
*Setup instructions for a gitian build of Cash using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cash
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-cash-host/README.md).
...
- Storage on Physical hard drive: Cashally Allocated 
...
Clone the git repositories for cash and gitian and then checkout the cash version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/zero-dynamics/cash-core.git
cd cash
...
Follow the instructions in [doc/release-process.md](release-process.md) in the cash repository
...
To build Cash (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the cash repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cash/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/zero-dynamics/cash-core
...
For example:
```bash
URL=https://github.com/crowning-/cash.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cash=${COMMIT} --url cash=${URL} ../cash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cash=${COMMIT} --url cash=${URL} ../cash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cash=${COMMIT} --url cash=${URL} ../cash/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cash-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cash-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/cash-build.assert
```
