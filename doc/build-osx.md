...
====================================
This guide will show you how to build odyncashd (headless client) for OSX.
...
------------------------
You can ignore this section if you are building `odyncashd` for your own use.

odyncashd/odyncash-cli binaries are not included in the OdynCash-Qt.app bundle.

If you are building `odyncashd` or `OdynCash-Qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the OdynCash-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./odyncashd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./odyncashd` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=odyncashrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/OdynCash/odyncash.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/OdynCash/odyncash.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/OdynCash/debug.log

Other commands:
-------

    ./odyncashd -daemon # to start the odyncash daemon.
    ./odyncash-cli --help  # for a list of command-line options.
    ./odyncash-cli help    # When the daemon is running, to get a list of RPC commands
