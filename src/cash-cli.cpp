...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help") || IsArgSet("-version")) {
        std::string strUsage = _("Cash RPC client version") + " " + FormatFullVersion() + "\n";
        if (!IsArgSet("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  cash-cli [options] <command> [params]  " + _("Send command to Cash") + "\n" +
                        "  cash-cli [options] help                " + _("List commands") + "\n" +
                        "  cash-cli [options] help <command>      " + _("Get help for a command") + "\n";
    ...
    if (GetArg("-rpcpassword", "") == "") {
        ...
        if (!GetAuthCookie(&strRPCUserColonPass)) {
            throw std::runtime_error(strprintf(
                ...
                GetConfigFile(GetArg("-conf", CASH_CONF_FILENAME)).string().c_str()));
        }
    } else {
