...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Cash users should be well used to small numbers with many decimal places by now :)
        ...
        //
        {
            ...
            BOOST_FOREACH (CNode* pnode, vNodesCopy) {
                if (pnode->fDisconnect) {
                    LogPrint("net", "ThreadSocketHandler -- removing node: peer=%d addr=%s nRefCount=%d fInbound=%d fMasternode=%d\n",
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        std::string strDesc = "Cash " + FormatFullVersion();
