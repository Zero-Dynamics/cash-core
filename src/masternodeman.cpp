...
void CMasternodeMan::AskForMN(CNode* pnode, const COutPoint& outpoint, CConnman& connman)
{
    ...
    mWeAskedForMasternodeListEntry[outpoint][addrSquashed] = GetTime() + PSEG_UPDATE_SECONDS;
...
void CMasternodeMan::PsegUpdate(CNode* pnode, CConnman& connman)
{
    CNetMsgMaker msgMaker(pnode->GetSendVersion());
    LOCK(cs);
    ...
    if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
        if (!(pnode->addr.IsRFC1918() || pnode->addr.IsLocal())) {
            ...
            if (it != mWeAskedForMasternodeList.end() && GetTime() < (*it).second) {
                LogPrintf("CMasternodeMan::PsegUpdate -- we already asked %s for the list; skipping...\n", addrSquashed.ToString());
                return;
            }
        }
    }
    ...
    LogPrint("masternode", "CMasternodeMan::PsegUpdate -- asked %s for the list\n", pnode->addr.ToString());
}
...
void CMasternodeMan::ProcessMasternodeConnections(CConnman& connman)
{
    ...
    connman.ForEachNode(CConnman::AllNodes, [&vecMnInfo](CNode* pnode) {
        if (pnode->fMasternode) {
            ...
            if (fFound)
                return; // do NOT disconnect mixing masternodes
#endif                  // ENABLE_WALLET
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::MNANNOUNCE) { //Masternode Broadcast
    ...
    } else if (strCommand == NetMsgType::PSEG) { //Get Masternode list or specific entry
        ...
        LogPrint("masternode", "PSEG -- Masternode list, Masternode=%s\n", masternodeOutpoint.ToStringShort());
...
void CMasternodeMan::SyncSingle(CNode* pnode, const COutPoint& outpoint, CConnman& connman)
{
    ...
    if (it != mapMasternodes.end()) {
        if (it->second.addr.IsRFC1918() || it->second.addr.IsLocal())
            return; // do not send local network Masternode
...
void CMasternodeMan::SyncAll(CNode* pnode, CConnman& connman)
{
    ...
    for (const auto& mnpair : mapMasternodes) {
        ...
        PushPsegInvs(pnode, mnpair.second);
        nInvCount++;
    }
...
void CMasternodeMan::PushPsegInvs(CNode* pnode, const CMasternode& mn)
{
    AssertLockHeld(cs);
...
{
    if (activeMasternode.outpoint == COutPoint())
        return;
    if (!masternodeSync.IsSynced())
        return;
...
{
    if (!masternodeSync.IsSynced() || mapMasternodes.empty())
        return;
...
void CMasternodeMan::ProcessVerifyBroadcast(CNode* pnode, const CMasternodeVerification& mnv)
{
...

    {
        ...
        if (!pmn1) {
            LogPrint("masternode", "CMasternodeMan::ProcessVerifyBroadcast -- can't find Masternode1 %s\n", mnv.masternodeOutpoint1.ToStringShort());
            return;
        }

        CMasternode* pmn2 = Find(mnv.masternodeOutpoint2);
        if (!pmn2) {
            LogPrint("masternode", "CMasternodeMan::ProcessVerifyBroadcast -- can't find Masternode %s\n", mnv.masternodeOutpoint2.ToStringShort());
            return;
        }

        if (pmn1->addr != mnv.addr) {
            LogPrint("masternode", "CMasternodeMan::ProcessVerifyBroadcast -- addr %s does not match %s\n", mnv.addr.ToString(), pmn1->addr.ToString());
            return;
        }
        ...
        if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
            ...
            if (!CHashSigner::VerifyHash(hash1, pmn1->pubKeyMasternode, mnv.vchSig1, strError)) {
                LogPrint("masternode", "CMasternodeMan::ProcessVerifyBroadcast -- VerifyHash() failed, error: %s\n", strError);
                return;
            }
            ...
            if (!CMessageSigner::VerifyMessage(pmn1->pubKeyMasternode, mnv.vchSig1, strMessage1, strError)) {
                LogPrint("masternode", "CMasternodeMan::ProcessVerifyBroadcast -- VerifyMessage() for masternode1 failed, error: %s\n", strError);
                return;
            }

            if (!CMessageSigner::VerifyMessage(pmn2->pubKeyMasternode, mnv.vchSig2, strMessage2, strError)) {
                LogPrint("masternode", "CMasternodeMan::ProcessVerifyBroadcast -- VerifyMessage() for masternode2 failed, error: %s\n", strError);
                return;
            }
        }
        ...
        LogPrint("masternode", "CMasternodeMan::ProcessVerifyBroadcast -- verified Masternode %s for addr %s\n",
            pmn1->outpoint.ToStringShort(), pmn1->addr.ToString());

        // increase ban score for everyone else with the same addr
        int nCount = 0;
        for (auto& mnpair : mapMasternodes) {
            if (mnpair.second.addr != mnv.addr || mnpair.first == mnv.masternodeOutpoint1)
                continue;
        ...
        if (nCount)
            LogPrint("masternode", "CMasternodeMan::ProcessVerifyBroadcast -- PoSe score increased for %d fake Masternodes, addr %s\n",
...
std::string CMasternodeMan::ToString() const
{
    ...
    info << "Masternodes: " << (int)mapMasternodes.size() << ", peers who asked us for Masternode list: " << (int)mAskedUsForMasternodeList.size() << ", peers we asked for Masternode list: " << (int)mWeAskedForMasternodeList.size() << ", entries in Masternode list we asked for: " << (int)mWeAskedForMasternodeListEntry.size() << ", nPsqCount: " << (int)nPsqCount;
...
void CMasternodeMan::UpdateLastPaid(const CBlockIndex* pindex)
{
    ...
    if (fLiteMode || !masternodeSync.IsWinnersListSynced() || mapMasternodes.empty())
        return;
