...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total odyncash balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized odyncash balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("debug", "odyncash") + HelpExampleRpc("debug", "odyncash+net"));
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given odyncash address.\n"
            "\nArguments:\n"
            "1. \"odyncashaddress\"     (string, required) The odyncash address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"odyncashaddress\", (string) The odyncash address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: OdynCash address and we have full public key:
        COdynCashAddress address(ks);
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2) {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
                          ...
                          "2. \"keys\"       (string, required) A json array of keys which are odyncash addresses or hex-encoded public keys\n"
                          "     [\n"
                          "       \"key\"    (string) odyncash address or hex-encoded public key\n"
                          "       ,...\n"
                          "     ]\n"
    ...
    COdynCashAddress address(innerID);
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"odyncashaddress\"  (string, required) The odyncash address to use for the signature.\n"
...
{
    if (type == 2) {
        address = COdynCashAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = COdynCashAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        COdynCashAddress address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            COdynCashAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
static const CRPCCommand commands[] =
    {
        ...
        /* OdynCash features */
        {"odyncash", "snsync", &snsync, true, {}},
        {"odyncash", "spork", &spork, true, {"value"}},
