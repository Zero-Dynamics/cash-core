...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp ||
        ...
        throw std::runtime_error(
            ...
            "  vote-conf          - Vote on a governance object by masternode configured in cash.conf\n"
    ...
    */
    ...
    if (strCommand == "vote-conf") {
        ...
        std::vector<unsigned char> vchMasternodeModeSignature;
        std::string strMasternodeModeSignMessage;
        ...
        if (!fMnFound) {
            ...
            resultsObj.push_back(Pair("cash.conf", statusObj));
...
static const CRPCCommand commands[] =
    {
        ...
        //  ---------------------    ------------------------  -----------------------  ------ ---
        /* Cash features */
        {"cash", "getgovernanceinfo", &getgovernanceinfo, true, {}},
        {"cash", "getsuperblockbudget", &getsuperblockbudget, true, {"index"}},
        {"cash", "gobject", &gobject, true, {}},
        {"cash", "voteraw", &voteraw, true, {}},
