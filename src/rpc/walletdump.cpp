...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"cashprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importstealthaddress(const JSONRPCRequest& request)
{
    ...
    CCashSecret vchScanSecret, vchSpendSecret;
...
UniValue importmnemonic(const JSONRPCRequest& request)
{
    ...
    entry.push_back(Pair("Done", "Stopping daemon... Please restart cash..."));
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (dest.type() == typeid(CStealthAddress)) {
        ...
        result.push_back(Pair("scan_private_key", CCashSecret(sxAddr.scan_secret).ToString()));
        ...
        if (!pwalletMain->GetKey(keyID, vchSecret))
            throw JSONRPCError(RPC_WALLET_ERROR, "Private key for address " + strAddress + " is not known");
        return CCashSecret(vchSecret).ToString();
    }
...
UniValue dumpbdapkeys(const JSONRPCRequest& request)
{
    ...
    std::string strWalletPrivKey = CCashSecret(vchWalletSecret).ToString();
    ...
    std::string strLinkPrivKey = CCashSecret(vchLinkSecret).ToString();
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Cash %s\n", CLIENT_BUILD);
    ...
    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("cashversion", CLIENT_BUILD));
