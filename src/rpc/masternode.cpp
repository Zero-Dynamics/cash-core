...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "current" || strCommand == "winner") {
        ...
        obj.push_back(Pair("payee", CDebitAddress(mnInfo.pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "outputs") {
        ...
        pwalletMain->AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_15000);
    ...
    if (strCommand == "status") {
        ...
        if (mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.push_back(Pair("payee", CDebitAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
...
UniValue masternodelist(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (strMode != "activeseconds" && strMode != "addr" && strMode != "daemon" && strMode != "full" && strMode != "info" && strMode != "json" &&
        ...
        throw std::runtime_error(
            ...
            "  payee          - Print Cash debit address associated with a masternode (can be additionally filtered,\n"
            "                   partial match)\n"
    ...
    if (strMode == "rank") {
        ...
        for (const auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) << mn.GetStatus() << " " << mn.nProtocolVersion << " " << CDebitAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " << (int64_t)mn.lastPing.sigTime << " " << std::setw(8) << (int64_t)(mn.lastPing.sigTime - mn.sigTime) << " " << std::setw(10) << mn.GetLastPaidTime() << " " << std::setw(6) << mn.GetLastPaidBlock() << " " << mn.addr.ToString();
                ...
                streamInfo << std::setw(18) << mn.GetStatus() << " " << mn.nProtocolVersion << " " << CDebitAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " << (int64_t)mn.lastPing.sigTime << " " << std::setw(8) << (int64_t)(mn.lastPing.sigTime - mn.sigTime) << " " << mn.lastPing.GetSentinelString() << " " << (mn.lastPing.fSentinelIsCurrent ? "current" : "expired") << " " << mn.addr.ToString();
                ...
                std::ostringstream streamInfo;
                streamInfo << mn.addr.ToString() << " " << CDebitAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " << mn.GetStatus() << " " << mn.nProtocolVersion << " " << mn.lastPing.nDaemonVersion << " " << mn.lastPing.GetSentinelString() << " " << (mn.lastPing.fSentinelIsCurrent ? "current" : "expired") << " " << (int64_t)mn.lastPing.sigTime << " " << (int64_t)(mn.lastPing.sigTime - mn.sigTime) << " " << mn.GetLastPaidTime() << " " << mn.GetLastPaidBlock();
                std::string strInfo = streamInfo.str();
                if (strFilter != "" && strInfo.find(strFilter) == std::string::npos &&
                    strOutpoint.find(strFilter) == std::string::npos)
                    continue;
                ...
                objMN.push_back(Pair("payee", CDebitAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
                ...
                CDebitAddress address(mn.pubKeyCollateralAddress.GetID());
    ...
    if (strCommand == "decode") {
        ...
        for (const auto& mnb : vecMnb) {
            ...
            if (mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CDebitAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CDebitAddress(mnb.pubKeyMasternode.GetID()).ToString()));
                resultObj.push_back(Pair("vchSig", EncodeBase64(&mnb.vchSig[0], mnb.vchSig.size())));
                resultObj.push_back(Pair("sigTime", mnb.sigTime));
                resultObj.push_back(Pair("protocolVersion", mnb.nProtocolVersion));
                resultObj.push_back(Pair("nLastPsq", mnb.nLastPsq));
...
static const CRPCCommand commands[] =
    {
        ...
        //  ---------------------    ----------------------   -------------------  ------ ----
        /* Cash features */
        {"cash", "masternode", &masternode, true, {}},
        {"cash", "masternodelist", &masternodelist, true, {}},
        {"cash", "masternodebroadcast", &masternodebroadcast, true, {}},
