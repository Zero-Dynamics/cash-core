...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), CASH_CONF_FILENAME));
    if (mode == HMM_CASHD) {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), CASH_PID_FILENAME));
#endif
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2024-%i Zero Dynamics Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Cash is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Cash process is using the data directory.
    ...
    try {
        ...
        if (!lock.try_lock()) {
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Cash is probably already running."), strDataDir));
        }
        if (probeOnly) {
            lock.unlock();
        }
    } catch (const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Cash is probably already running.") + " %s.", strDataDir, e.what()));
    }
    return true;
}
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", CASH_CONF_FILENAME)).string());
    ...
    if (fLiteMode) {
        InitWarning(_("You are starting in lite mode, all Cash-specific functionality is disabled."));
    }
    ...
    // ********************************************************* Step 11d: start cash-ps-<smth> threads
