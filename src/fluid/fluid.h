...
public:
    ...
    std::vector<std::pair<std::string, COdynCashAddress> > InitialiseSovereignIdentities();
...
public:
    ...
    bool GenericConsentMessage(const std::string& message, std::string& signedString, const COdynCashAddress& signer);
    bool GenericParseNumber(const std::string consentToken, const int64_t timeStamp, CAmount& howMuch, bool txCheckPurpose = false);
    bool GenericVerifyInstruction(const std::string& consentToken, COdynCashAddress& signer, std::string& messageTokenKey, const int& whereToLook = 1);

    bool ExtractCheckTimestamp(const std::string& strOpCode, const std::string& consentToken, const int64_t& timeStamp);
    bool ParseMintKey(const int64_t& nTime, COdynCashAddress& destination, CAmount& coinAmount, const std::string& uniqueIdentifier, const bool txCheckPurpose = false);
    bool ProcessFluidToken(const std::string& consentToken, std::vector<std::string>& ptrs, const int& strVecNo);

    bool GetMintingInstructions(const CBlockIndex* pblockindex, COdynCashAddress& toMintAddress, CAmount& mintAmount);
    ...
    COdynCashAddress GetAddressFromDigestSignature(const std::string& digestSignature, const std::string& messageTokenKey);
