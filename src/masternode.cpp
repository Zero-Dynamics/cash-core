...
void CMasternode::UpdateLastPaid(const CBlockIndex* pindex, int nMaxBlocksToScanBack)
{
    ...
    for (int i = 0; BlockReading && BlockReading->nHeight > nBlockLastPaid && i < nMaxBlocksToScanBack; i++) {
        if (mnpayments.mapMasternodeBlocks.count(BlockReading->nHeight) &&
            ...
            CAmount nMasternodePayment = GetFluidMasternodeReward(BlockReading->nHeight);
...
bool CMasternodeBroadcast::SimpleCheck(int& nDos)
{
    ...
    if (!IsValidNetAddr()) {
        LogPrint("masternode", "CMasternodeBroadcast::SimpleCheck -- Invalid addr, rejected: Masternode=%s  addr=%s\n",
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrint("masternode", "CMasternodeBroadcast::SimpleCheck -- Signature rejected, too far into the future: Masternode=%s\n", outpoint.ToStringShort());
        nDos = 1;
        return false;
    }
    ...
    if (nProtocolVersion < mnpayments.GetMinMasternodePaymentsProto()) {
        LogPrint("masternode", "CMasternodeBroadcast::SimpleCheck -- outdated Masternode: Masternode=%s  nProtocolVersion=%d\n", outpoint.ToStringShort(), nProtocolVersion);
        nActiveState = MASTERNODE_UPDATE_REQUIRED;
    }
    ...
    if (pubkeyScript.size() != 25) {
        LogPrint("masternode", "CMasternodeBroadcast::SimpleCheck -- pubKeyCollateralAddress has the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    if (pubkeyScript2.size() != 25) {
        LogPrint("masternode", "CMasternodeBroadcast::SimpleCheck -- pubKeyMasternode has the wrong size\n");
        nDos = 100;
        return false;
    }
...
bool CMasternodeBroadcast::Update(CMasternode* pmn, int& nDos, CConnman& connman)
{
    ...
    if (pmn->sigTime > sigTime) {
        LogPrint("masternode", "CMasternodeBroadcast::Update -- Bad sigTime %d (existing broadcast is at %d) for Masternode %s %s\n",
    ...
    if (pmn->IsPoSeBanned()) {
        LogPrint("masternode", "CMasternodeBroadcast::Update -- Banned by PoSe, Masternode=%s\n", outpoint.ToStringShort());
        return false;
    }

    // IsVnAssociatedWithPubkey is validated once in CheckOutpoint, after that they just need to match
    if (pmn->pubKeyCollateralAddress != pubKeyCollateralAddress) {
        LogPrint("masternode", "CMasternodeBroadcast::Update -- Got mismatched pubKeyCollateralAddress and vin\n");
        nDos = 33;
        return false;
    }

    if (!CheckSignature(nDos)) {
        LogPrint("masternode", "CMasternodeBroadcast::Update -- CheckSignature() failed, masternode=%s\n", outpoint.ToStringShort());
        return false;
    }
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if (err == COLLATERAL_INVALID_AMOUNT) {
        LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 0DYNC, masternode=%s\n", outpoint.ToStringShort());
        nDos = 33;
        return false;
    }
    ...
    if (pRequredConfIndex->GetBlockTime() > sigTime) {
        LogPrintf("CMasternodeBroadcast::CheckOutpoint -- Bad sigTime %d (%d conf block is at %d) for Masternode %s %s\n",
            sigTime, Params().GetConsensus().nMasternodeMinimumConfirmations, pRequredConfIndex->GetBlockTime(), outpoint.ToStringShort(), addr.ToString());
        return false;
    }
...
bool CMasternodePing::CheckAndUpdate(CMasternode* pmn, bool fFromNewBroadcast, int& nDos, CConnman& connman)
{
    ...
    if (!pmn->IsEnabled() && !pmn->IsExpired() && !pmn->IsSentinelPingExpired())
        return false;
