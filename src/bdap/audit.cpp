...
bool BuildAuditJson(const CAudit& audit, UniValue& oAudit)
{
    ...
    for(const std::vector<unsigned char>& vchAudit : auditData.vAuditData) {
        counter++;
        oAuditHashes.push_back(makeUniValuePair("audit_hash" + std::to_string(counter), stringFromVch(vchAudit)));
    }
    oAudit.push_back(makeUniValuePair("version", std::to_string(audit.Version())));
    oAudit.push_back(makeUniValuePair("audit_count", std::to_string(auditData.vAuditData.size())));
    oAudit.push_back(makeUniValuePair("audit_hashes", oAuditHashes));
    oAudit.push_back(makeUniValuePair("timestamp", std::to_string(auditData.nTimeStamp)));
    oAudit.push_back(makeUniValuePair("owner", stringFromVch(audit.vchOwnerFullObjectPath)));
    oAudit.push_back(makeUniValuePair("signed", audit.IsSigned() ? "True" : "False"));
    oAudit.push_back(makeUniValuePair("algorithm_type", stringFromVch(audit.vchAlgorithmType)));
    oAudit.push_back(makeUniValuePair("description", stringFromVch(audit.vchDescription)));
    oAudit.push_back(makeUniValuePair("txid", audit.txHash.GetHex()));
    ...
    oAudit.push_back(makeUniValuePair("block_time", std::to_string(nTime)));
    oAudit.push_back(makeUniValuePair("block_height", std::to_string(audit.nHeight)));
    return true;
}
