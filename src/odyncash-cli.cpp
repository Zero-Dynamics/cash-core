...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help") || IsArgSet("-version")) {
        std::string strUsage = _("OdynCash RPC client version") + " " + FormatFullVersion() + "\n";
        if (!IsArgSet("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  odyncash-cli [options] <command> [params]  " + _("Send command to OdynCash") + "\n" +
                        "  odyncash-cli [options] help                " + _("List commands") + "\n" +
                        "  odyncash-cli [options] help <command>      " + _("Get help for a command") + "\n";
    ...
    if (GetArg("-rpcpassword", "") == "") {
        ...
        if (!GetAuthCookie(&strRPCUserColonPass)) {
            throw std::runtime_error(strprintf(
                ...
                GetConfigFile(GetArg("-conf", ODYNCASH_CONF_FILENAME)).string().c_str()));
        }
    } else {
