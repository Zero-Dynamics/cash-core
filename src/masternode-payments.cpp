...
bool IsBlockPayeeValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
    if (!masternodeSync.IsSynced()) {
        ...
        LogPrint("mnpayments", "IsBlockPayeeValid -- WARNING: Client not synced, skipping block payee checks\n");
        return true;
    }
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet) const
{
    ...
    if (hasPayment && !mnpayments.GetBlockPayee(nBlockHeight, payee)) {
    ...
    // make sure it's not filled yet
    txoutMasternodeRet = CTxOut();
    CAmount masternodePayment = GetFluidMasternodeReward(nBlockHeight);
    ...
    LogPrintf("CMasternodePayments::FillBlockPayee -- Masternode payment %ld to %s\n", FormatMoney(masternodePayment), address2.ToString());
}

int CMasternodePayments::GetMinMasternodePaymentsProto() const
{
    return sporkManager.IsSporkActive(SPORK_10_MASTERNODE_PAY_UPDATED_NODES) ? MIN_MASTERNODE_PAYMENT_PROTO_VERSION_2 : MIN_MASTERNODE_PAYMENT_PROTO_VERSION_1;
}
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew, const int nHeight) const
{
    LOCK(cs_vecPayees);

    int nMaxSignatures = 0;
    std::string strPayeesPossible = "";
    CAmount nMasternodePayment = GetFluidMasternodeReward(nHeight);
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f 0DYNC\n", strPayeesPossible, (float)nMasternodePayment / COIN);
    return false;
}
...
bool CMasternodePaymentVote::IsValid(CNode* pnode, int nValidationHeight, std::string& strError, CConnman& connman) const
{
    ...
    if (!fMasternodeMode && nBlockHeight < nValidationHeight)
        return true;
...
void CMasternodePayments::CheckBlockVotes(int nBlockHeight)
{
    ...
    for (const auto& mn : mns) {
        ...
        if (++i >= MNPAYMENTS_SIGNATURES_TOTAL)
            break;
    }
