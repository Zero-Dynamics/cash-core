...
const std::string CashGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
    "macosx"
...
CashGUI::CashGUI(const PlatformStyle* _platformStyle, const NetworkStyle* networkStyle, QWidget* parent) : QMainWindow(parent),
                                                                                                                 enableWallet(false),
                                                                                                                 ...
                                                                                                                 platformStyle(_platformStyle)
{
    ...
    GUIUtil::restoreWindowGeometry("nWindow", QSize(1062, 563), this);
...
void CashGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and cash: URIs)"));
    ...
    aboutAction = new QAction(QIcon(":/icons/" + theme + "/about"), tr("&About Cash"), this);
...
void CashGUI::setClientModel(ClientModel* _clientModel)
{
...
#ifdef ENABLE_WALLET
bool CashGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    if (!walletFrame)
        return false;
...
bool CashGUI::setCurrentWallet(const QString& name)
{
    if (!walletFrame)
        return false;
...
void CashGUI::setWalletActionsEnabled(bool enabled)
{
...
void CashGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Cash client") + " " + networkStyle->getTitleAddText();
...
void CashGUI::createIconMenu(QMenu* pmenu)
{
...
#ifndef Q_OS_MAC
void CashGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void CashGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void CashGUI::showPrivateSendHelpClicked()
{
    if (!clientModel)
        return;
...
void CashGUI::gotoSendCoinsPage(QString addr)
{
...
void CashGUI::gotoReceiveCoinsPage()
{
...
void CashGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame)
        walletFrame->gotoSignMessageTab(addr);
}

void CashGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame)
...
void CashGUI::setNumConnections(int count)
{
    QString icon;
    QString theme = GUIUtil::getThemeName();
    switch (count) {
...
void CashGUI::updateHeadersSyncProgressLabel()
{
...
void CashGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay) {
        if (header)
...
void CashGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if (!clientModel)
        return;
...
void CashGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
void CashGUI::changeEvent(QEvent* e)
{
...
void CashGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if (clientModel && clientModel->getOptionsModel()) {
...
void CashGUI::showEvent(QShowEvent* event)
{
...
#ifdef ENABLE_WALLET
void CashGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    ...
    message((amount) < 0 ? (tr("Sent transaction")) : tr("Incoming transaction"),
        tr("Date: %1\n"
           ...
           "Address: %4\n")
            .arg(date)
            .arg(CashUnits::formatWithUnit(unit, amount, true))
            .arg(type)
            .arg(address),
...
void CashGUI::dragEnterEvent(QDragEnterEvent* event)
{
    // Accept only URIs
...
void CashGUI::dropEvent(QDropEvent* event)
{
    if (event->mimeData()->hasUrls()) {
...
bool CashGUI::eventFilter(QObject* object, QEvent* event)
{
...
#ifdef ENABLE_WALLET
bool CashGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void CashGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void CashGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch (status) {
...
void CashGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void CashGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CashGUI::detectShutdown()
{
...
void CashGUI::showProgress(const QString& title, int nProgress)
{
    if (nProgress == 0) {
...
void CashGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon) {
...
static bool ThreadSafeMessageBox(CashGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void CashGUI::subscribeToCoreSignals()
{
...
void CashGUI::unsubscribeFromCoreSignals()
{
...
void CashGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
                                                                                               ...
                                                                                               menu(0)
{
    ...
    QList<CashUnits::Unit> units = CashUnits::availableUnits();
    int max_width = 0;
...
void UnitDisplayStatusBarControl::createContextMenu()
{
    ...
    Q_FOREACH (CashUnits::Unit u, CashUnits::availableUnits()) {
        QAction* menuAction = new QAction(QString(CashUnits::name(u)), this);
