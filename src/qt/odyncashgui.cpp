...
const std::string OdynCashGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
    "macosx"
...
const QString OdynCashGUI::DEFAULT_WALLET = "~Default";

OdynCashGUI::OdynCashGUI(const PlatformStyle* _platformStyle, const NetworkStyle* networkStyle, QWidget* parent) : QMainWindow(parent),
                                                                                                                 enableWallet(false),
                                                                                                                 ...
                                                                                                                 platformStyle(_platformStyle)
{
    ...
    QString windowTitle = tr("OdynCash") + " - ";
#ifdef ENABLE_WALLET
...
void OdynCashGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a OdynCash address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and odyncash: URIs)"));
    ...
    aboutAction = new QAction(QIcon(":/icons/" + theme + "/about"), tr("&About OdynCash"), this);
    aboutAction->setStatusTip(tr("Show information about OdynCash"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for OdynCash"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your OdynCash addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified OdynCash addresses"));
    ...
    openAction->setStatusTip(tr("Open a odyncash: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the OdynCash help message to get a list with possible OdynCash command-line options"));
...
void OdynCashGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void OdynCashGUI::createToolBars()
{
#ifdef ENABLE_WALLET
    if (walletFrame) {
...
void OdynCashGUI::setClientModel(ClientModel* _clientModel)
{
...
#ifdef ENABLE_WALLET
bool OdynCashGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    if (!walletFrame)
        return false;
...
bool OdynCashGUI::setCurrentWallet(const QString& name)
{
    if (!walletFrame)
        return false;
...
void OdynCashGUI::removeAllWallets()
{
    if (!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void OdynCashGUI::setWalletActionsEnabled(bool enabled)
{
...
void OdynCashGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("OdynCash client") + " " + networkStyle->getTitleAddText();
...
void OdynCashGUI::createIconMenu(QMenu* pmenu)
{
...
#ifndef Q_OS_MAC
void OdynCashGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void OdynCashGUI::optionsClicked()
{
...
void OdynCashGUI::showRPCConsole()
{
...
void OdynCashGUI::showInfo()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showRPCConsole();
}

void OdynCashGUI::showConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showRPCConsole();
}

void OdynCashGUI::showGraph()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showRPCConsole();
}

void OdynCashGUI::showPeers()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showRPCConsole();
}

void OdynCashGUI::showRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showRPCConsole();
}

void OdynCashGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void OdynCashGUI::showDNConfEditor()
{
    GUIUtil::openDNConfigfile();
}

void OdynCashGUI::showBackups()
{
    GUIUtil::showBackups();
}

void OdynCashGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void OdynCashGUI::showPrivateSendHelpClicked()
{
    if (!clientModel)
        return;
...
#ifdef ENABLE_WALLET
void OdynCashGUI::mnemonicClicked()
{
...
void OdynCashGUI::gotoOverviewPage()
{
...
void OdynCashGUI::gotoSendCoinsPage(QString addr)
{
...
void OdynCashGUI::gotoReceiveCoinsPage()
{
...
void OdynCashGUI::gotoHistoryPage()
{
...
void OdynCashGUI::gotoServiceNodePage()
{
...
void OdynCashGUI::gotoMiningPage()
{
...
void OdynCashGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame)
        walletFrame->gotoSignMessageTab(addr);
}

void OdynCashGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame)
...
void OdynCashGUI::setNumConnections(int count)
{
    QString icon;
    QString theme = GUIUtil::getThemeName();
    switch (count) {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to OdynCash network", "", count));
}

void OdynCashGUI::updateHeadersSyncProgressLabel()
{
...
void OdynCashGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay) {
        if (header)
...
void OdynCashGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if (!clientModel)
        return;
...
void OdynCashGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
    QString strTitle = tr("OdynCash"); // default title
...
void OdynCashGUI::changeEvent(QEvent* e)
{
...
void OdynCashGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if (clientModel && clientModel->getOptionsModel()) {
...
void OdynCashGUI::showEvent(QShowEvent* event)
{
...
#ifdef ENABLE_WALLET
void OdynCashGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    ...
    message((amount) < 0 ? (tr("Sent transaction")) : tr("Incoming transaction"),
        tr("Date: %1\n"
           ...
           "Address: %4\n")
            .arg(date)
            .arg(OdynCashUnits::formatWithUnit(unit, amount, true))
            .arg(type)
            .arg(address),
...
void OdynCashGUI::dragEnterEvent(QDragEnterEvent* event)
{
    // Accept only URIs
...
void OdynCashGUI::dropEvent(QDropEvent* event)
{
    if (event->mimeData()->hasUrls()) {
...
bool OdynCashGUI::eventFilter(QObject* object, QEvent* event)
{
...
#ifdef ENABLE_WALLET
bool OdynCashGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void OdynCashGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void OdynCashGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch (status) {
...
void OdynCashGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void OdynCashGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void OdynCashGUI::detectShutdown()
{
...
void OdynCashGUI::showProgress(const QString& title, int nProgress)
{
    if (nProgress == 0) {
...
void OdynCashGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon) {
...
void OdynCashGUI::showModalOverlay()
{
...
static bool ThreadSafeMessageBox(OdynCashGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void OdynCashGUI::subscribeToCoreSignals()
{
...
void OdynCashGUI::unsubscribeFromCoreSignals()
{
...
void OdynCashGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
                                                                                               ...
                                                                                               menu(0)
{
    ...
    QList<OdynCashUnits::Unit> units = OdynCashUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const OdynCashUnits::Unit unit, units) {
        max_width = qMax(max_width, fm.width(OdynCashUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH (OdynCashUnits::Unit u, OdynCashUnits::availableUnits()) {
        QAction* menuAction = new QAction(QString(OdynCashUnits::name(u)), this);
...
{
    setPixmap(QIcon(":/icons/drk/unit_" + OdynCashUnits::id(newUnits)).pixmap(33, STATUSBAR_ICONSIZE));
}
