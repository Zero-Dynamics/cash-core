...
void ServiceNodeList::updateMyServiceNodeInfo(QString strAlias, QString strAddr, const COutPoint& outpoint)
{
    ...
    QTableWidgetItem* pubkeyItem = new QTableWidgetItem(QString::fromStdString(fFound ? COdynCashAddress(infoDn.pubKeyCollateralAddress.GetID()).ToString() : ""));
...
void ServiceNodeList::updateNodeList()
{
    ...
    for (const auto& dnpair : mapServiceNodes) {
        ...
        QTableWidgetItem* pubkeyItem = new QTableWidgetItem(QString::fromStdString(COdynCashAddress(dn.pubKeyCollateralAddress.GetID()).ToString()));
